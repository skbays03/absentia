"""Absentia's TUI — primary exploration interface.

The TUI is launched by running ``absentia`` with no subcommand from a TTY.
...
The engine is shared with ``absentia check``: the TUI calls
...
"""
from .app import AbsentiaApp, run_tui

__all__ = ["AbsentiaApp", "run_tui"]
